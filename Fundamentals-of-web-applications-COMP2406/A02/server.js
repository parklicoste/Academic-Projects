...
http.createServer(function(request, response) {
    ...
    request.on("end", function() {
      ...
      }else if(request.method === "POST" && urlObj.pathname === "/mySavedSong"){
        let fileLines = []
        var songLines = dataObj.songLines;
        var mySavedFileName = dataObj.saveAsFileName;
        var mySavedFilePath = "songs/" + mySavedFileName + ".txt";
        var fileDataString = "";
        for (var i = 0; i < songLines.length; i++) {
          fileDataString += songLines[i];
          if (i < songLines.length - 1) fileDataString += "\n";
        }
        fs.writeFile(mySavedFilePath, fileDataString, function(err, data) {
          var returnObj = {};
          returnObj.text = "Saved: " + mySavedFilePath;
          response.writeHead(200, { "Content-Type": MIME_TYPES["json"] });
          response.end(JSON.stringify(returnObj));
          fileLines[mySavedFileName] = mySavedFilePath;
        });
